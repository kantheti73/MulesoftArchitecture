...
| [10 — Redis Cache for Flex Gateway](docs/10-redis-cache.md) | What Flex Gateway uses Redis for and what it doesn't (config NO, body cache NO, distributed rate limiting YES). Decision rubric. Deployment options by platform (ElastiCache / Azure Cache / Sentinel / Enterprise). On-prem Sentinel 3-node install (RHEL config + TLS + AUTH). Flex Gateway YAML to point rate-limit policy at Redis with `fallback-on-error: local`. DR behavior table. Active/active multi-DC honesty (Redis Enterprise is the only clean answer; alternatives + their trade-offs documented). Anti-patterns. |
